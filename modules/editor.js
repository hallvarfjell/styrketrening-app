...
const Editor = {
  render(existing = null) {
    ...
    const rest = Array.from(noiseSet).filter(v => !preferNoise.includes(v)).sort((a,b)=>String(a).localeCompare(String(b)));
    ...
    document.getElementById('eximport').onclick=()=>{
      ...
      const r=new FileReader(); r.onload=()=>{
        ...
        AppState.exercises=rows.map(x=>{
          const desc=(x.description||''); const prog=(x.progression_tips||'').trim();
    ...
    document.getElementById('exportEx').onclick=()=>{
      ...
      const rows=(AppState.exercises||[]).map(e=>{
        ...
        return [
          ...
          e.name,
          e.description,
          e.default_duration_sec,      // <- RIKTIG FELT (ikke ...Klar!)
    ...
    function renderItems(){
      ...
      w.items.forEach((it, idx) => {
        ...
        html +=   '<div class="flex">';
        html +=     '<div style="flex:1;"><div class="small" style="color:#666; margin-bottom:4px;">Varighet (mm:ss)</div><input class="input" value="'+Util.fmtMMSS(d)+'" data-dur="'+idx+'" /></div>';
        html +=     '<div style="flex:1;"><div class="small" style="color:#666; margin-bottom:4px;">Pause (s)</div><input class="input" value="'+p+'" data-pause="'+idx+'" /></div>';
        html +=   '</div>';
    ...
    function renderExercises(){
      ...
      for(const e of list){
        ...
        html+='<div class="small">'+(String(e.description||'').replace(/\n/g,'<br>'))+'</div>';
        html+='<div class="flex" style="margin-top:6px;"><button class="button" data-add="'+e.exercise_id+'">Legg til i økt ('+dur+'s)</button><button class="button secondary" data-edit="'+e.exercise_id+'">Rediger</button><button class="button secondary" data-del="'+e.exercise_id+'">Slett øvelse</button></div></div>';
      }
      document.getElementById('exlist').innerHTML=html||'<div class="card small">Ingen øvelser matcher filteret.</div>';
