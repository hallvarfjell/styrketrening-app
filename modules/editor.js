...
//
// Full Editor med:
// - Lik bredde på alle kort (styres av CSS; denne markupen følger samme grid).
// - Bygg økt: Fokusområde uten forhåndsverdi; Lagring via diskett-ikon.
// - Item-kort: tydelig drag-håndtak til høyre; Fjern = rødt søppelspann-ikon.
// - Lag/rediger øvelse: 2 kolonner × 3 rader for seks egenskaper; utstyr-velger med Velg alle + legg til nytt.
// - Øvelser (høyre): filter m/utstyrs-multivalg; pluss/blyant/søppelspann-ikoner på rad.
// - Import/eksport øvelser (inkl. default_pause_sec og intensitet).
...
const Editor = {
  render(existing = null) {
    const w = existing || {
      ...
      name: '',
      category: '',
      focus_area: '', // ingen forhåndsverdi
      favorite: false,
      items: [],
      created_at: Date.now()
    };
      ...
      '<div class="grid-2">' +
        '<div>' +
          ...
          '<div class="card">' +
            ...
            '<div class="flex" style="margin-top:8px;">' +
              '<input id="focus" class="input" placeholder="Fokusområde" list="focusList" value="'+(w.focus_area||'')+'" />' + // tom
          ...
          '<div class="card">' +
            ...
            // Utstyr-velger (multivalg) – dropdownpanel
        ...
        '<div>' +
          '<div class="card">' +
            ...
            // Utstyrsfilter – multivalg dropdown
            '<div class="multiselect" style="margin-top:8px;">' +
    ...
    function renderExercises(){
      ...
      // Rediger → fyller inn i venstre "Lag/rediger øvelse"
    ...
    // Init filter + lister
    function updateFilterEquipHint(){ const a=[...fSelectedEquip]; document.getElementById('fEqHint').textContent = a.length ? a.join(', ') : 'kun kroppsvekt'; }
    let fSelectedEquip = new Set();
