...
// modules/editor.js
//
// Stabil Editor m/ikon-helper (SVG <use href="#...">) for å unngå parse-feil.
// Funksjoner:
// - Bygg økt: fokusområde tomt; lagre via diskett-ikon.
// - Item-kort: drag-håndtak til høyre; Fjern (rødt søppelspann) på linje med feltene.
// - Tom liste: "Legg til med [pluss-ikon] i øvelseslista."
// - "Kopier serie": dupliserer hele økta og legger under.
// - Lag/rediger øvelse: 2×3 grid for 6 egenskaper; utstyr-velger (multivalg + legg til).
// - Øvelser (høyre): filter inkl. utstyr; pluss/blyant/søppelspann.
// - Import/eksport øvelser (inkl. default_pause_sec & intensitet).

const Editor = {
  render(existing = null) {
    // --- Helper for ikon-HTML ---
    const svg = (id, cls = 'icon') => '<svg class="' + cls + '"><use href="#' + id + '"></use></svg>';

    const w = existing || {
      workout_id: `WK${Date.now()}`,
      name: '',
      category: '',
      focus_area: '', // bevisst tom
      favorite: false,
      items: [],
      created_at: Date.now()
    };

    // --- Oppslagslister fra øvelser ---
    ...
    const optionsSimple = (arr, selectedVal) => arr.map(v => '<option' + (selectedVal===v?' selected':'') + '>' + v + '</option>').join('');
    const dataListOptions = (arr) => arr.map(v => '<option value="'+v+'">').join('');
      ...
      '<div class="grid-2">' +
        '<div>' +
          ...
          '<div class="card">' +
            ...
            '<div class="flex" style="margin-top:8px;">' +
              '<button class="icon-btn" id="save" aria-label="Lagre økt">'+ svg('ph-floppy-disk-fill') +'</button>' +
            '</div>' +
          '</div>' +
          ...
          '<div class="card">' +
            ...
            '<div class="form-grid-2" style="margin-top:8px;">' +
              ...
              '<select id="new_noise" class="input"><option value="">Lydnivå</option>' +
                allNoise.map(n => '<option value="'+n+'">'+n+'</option>').join('') +
              '</select>' +
            '</div>' +
        ...
        '<div>' +
          '<div class="card">' +
            ...
            '<div class="flex">' +
              ...
              '<select id="fCat" class="input"><option value="">Kategori (alle)</option>'+ optionsSimple(allCategories,'') +'</select>' +
            '</div>' +
            '<div class="flex" style="margin-top:8px;">' +
              ...
              '<select id="fNoise" class="input"><option value="">Lydnivå (alle)</option>' + allNoise.map(n => '<option value="'+n+'">'+n+'</option>').join('') + '</select>' +
            '</div>' +
    ...
    // --- Ny øvelse: utstyr-velger (venstre) ---
    ...
    const updateNewEquipHint = () => { const arr=[...newSelectedEquip]; newEqHint.textContent = arr.length ? arr.join(', ') : 'Kun kroppsvekt'; };
    ...
    document.getElementById('eximport').onclick = () => {
      ...
      r.onload = () => {
        ...
        AppState.exercises = rows.map(x=>{
          ...
          const rpeText = intens
            ? (intens==='Lav'?'Lett': intens==='Middels'?'Moderat':'Hardt')
    ...
    const updateFilterEquipHint = () => { const a=[...fSelectedEquip]; if (fEqHint) fEqHint.textContent = a.length ? a.join(', ') : 'kun kroppsvekt'; };
        ...
        html +=
          '<div class="card">' +
            '<div class="row">' +
              ...
              '</div>' +
              '<div class="actions">' +
                '<button class="icon-btn" aria-label="Legg til i økt" data-add="'+e.exercise_id+'">'+ svg('ph-plus') +'</button>' +
                '<button class="icon-btn" aria-label="Rediger" data-edit="'+e.exercise_id+'">'+ svg('ph-pencil-fill') +'</button>' +
                '<button class="icon-btn trash" aria-label="Slett" data-del="'+e.exercise_id+'">'+ svg('ph-trash-fill') +'</button>' +
              '</div>' +
            '</div>' +
          '</div>';
      }
    ...
    function renderItems(){
      ...
      if (!w.items || !w.items.length) {
        container.innerHTML =
          ...
          'Legg til med ' + svg('ph-plus','inline-icon') + ' i øvelseslista.</div>';
        return;
      }
        ...
        html +=
          '<div class="card item-card" data-idx="'+idx+'">' +
            ...
            '<div class="flex" style="margin-top:6px; align-items:flex-end;">' +
              ...
              '</div>' +
              '<div style="flex:0;">' +
                '<button class="icon-btn trash" aria-label="Fjern" data-del="'+idx+'">'+ svg('ph-trash-fill') +'</button>' +
              '</div>' +
            '</div>' +
          '</div>';
      });
