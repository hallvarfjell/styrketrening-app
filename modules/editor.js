...
const Editor = {
  render(existing = null) {
      ...
      '<div class="grid-2">' +
          ...
          '<div class="card">' +
            ...
            '<div class="multiselect" style="margin-top:8px;">' +
              ...
              '<div id="newEqMenu" class="card" style="display:none; position:relative; z-index:5; max-width:520px;">' +
                ...
                '</div><div style="margin-top:8px;"><input id="newEqAdd" class="input" placeholder="Legg til nytt utstyr og trykk Enter" /></div>' +
              '</div>' +
        ...
        '<div>' +
          '<div class="card">' +
            ...
            '<div class="flex"><select id="fFocus" class="input"><option value="">Fokus (alle)</option>'+ optionsSimple(allFocus,'') +'</select>' +
            '<select id="fCat" class="input"><option value="">Kategori (alle)</option>'+ (function(){let s=''; for(const c of allCategories){ s+='<option>'+c+'</option>'; } return s; })() +'</select></div>' +
            '<div class="flex" style="margin-top:8px;">' +
    ...
    document.getElementById('eximport').onclick=()=>{
      ...
      const r=new FileReader(); r.onload=()=>{
        ...
        AppState.exercises=rows.map(x=>{
          const desc=(x.description||''); const prog=(x.progression_tips||'').trim(); const full=prog?(desc+'\nProgresjon: '+prog):desc;
          const eq=(x.equipment||'').replace(/;/g,',').split(',').map(s=>s.trim()).filter(Boolean);
          const intens=(x.intensitet||'').trim(); // Lav/Middels/Høy
          let rpeText = intens ? (intens==='Lav'?'Lett': intens==='Middels'?'Moderat':'Hardt') : (x.rpe_text || (Number(x.rpe)<=3?'Lett':Number(x.rpe)<=6?'Moderat':'Hardt'));
    ...
    document.getElementById('exportEx').onclick=()=>{
      ...
      const csv=Util.toCSV(headers,rows,';'); Util.download('exercises.csv',csv,'text/csv');
    };
    ...
    document.getElementById('addExercise').onclick=()=>{
      ...
      const ex={
        exercise_id:'EX'+Date.now(), name, description:desc,
        default_duration_sec:/^\d+$/.test(dur)?Number(dur):60,
        default_pause_sec:/^\d+$/.test(pause)?Number(pause):10,
    ...
    let fSelectedEquip=new Set(); function updateFilterEquipHint(){ const arr=[...fSelectedEquip]; fEqHint.textContent=arr.length?arr.join(', '):'kun kroppsvekt'; }
    ...
    function matchesFilters(e){
      ...
      if(!sel.length && req.length) return false; if(sel.length && !req.every(x=>sel.includes(x))) return false;
    ...
    function renderExercises(){
      ...
      for(const e of list){
        ...
        html+='<div class="card">';
        html+='<div><strong>'+e.name+'</strong> <span class="small">'+(e.category||'(ingen kategori)')+' • '+(e.focus_area||'')+'</span></div>';
        html+='<div class="small">'+(String(e.description||'').replace(/\n/g,'<br>'))+'</div>';
        html+='<div class="flex" style="margin-top:6px;">';
        html+='<button class="button" data-add="'+e.exercise_id+'">Legg til i økt ('+dur+'s)</button>';
      ...
      document.querySelectorAll('[data-add]').forEach(b=>b.onclick=()=>{
        ...
        const dur=Number(ex?.default_duration_sec||60), pause=Number(ex?.default_pause_sec||10);
      ...
      document.querySelectorAll('[data-del]').forEach(b=>b.onclick=()=>{
        ...
        if(idx>=0 && confirm('Slette øvelsen?')){
          AppState.exercises.splice(idx,1); Store.save(Store.keys.exercises,AppState.exercises);
          (AppState.workouts||[]).forEach(W=>{ if(Array.isArray(W.items)) W.items=W.items.filter(it=>it.exercise_id!==eid); });
    ...
    function addDragHandlers(card, idx){
      ...
      card.addEventListener('drop', (e)=>{ e.preventDefault(); card.classList.remove('drag-over');
        ...
        if(Number.isFinite(from)&&from!==to){ const [mv]=w.items.splice(from,1); w.items.splice(to,0,mv); renderItems(); }
      });
    }
    ...
    function renderItems(){
      ...
      w.items.forEach((it,idx)=>{
        ...
        html+='<div class="flex">';
        html+='<div style="flex:1;"><div class="small" style="color:#666; margin-bottom:4px;">Varighet (mm:ss)</div><input class="input" value="'+Util.fmtMMSS(dur)+'" data-dur="'+idx+'"/></div>';
        html+='<div style="flex:1;"><div class="small" style="color:#666; margin-bottom:4px;">Pause (s)</div><input class="input" value="'+pause+'" data-pause="'+idx+'"/></div>';
        html+='</div><div class="flex" style="margin-top:8px;"><button class="button secondary" data-del="'+idx+'">Fjern</button></div></div>';
      });
    ...
    function renderExercises(){
      const list=(AppState.exercises||[]).filter(matchesFilters);
      list.sort((a,b)=>Number(b.created_at||0)-Number(a.created_at||0));
      let html=''; for(const e of list){ const dur=Number(e.default_duration_sec||60);
        html+='<div class="card"><div><strong>'+e.name+'</strong> <span class="small">'+(e.category||'(ingen kategori)')+' • '+(e.focus_area||'')+'</span></div>';
        html+='<div class="small">'+(String(e.description||'').replace(/\n/g,'<br>'))+'</div>';
        html+='<div class="flex" style="margin-top:6px;"><button class="button" data-add="'+e.exercise_id+'">Legg til i økt ('+dur+'s)</button>';
        html+='<button class="button secondary" data-edit="'+e.exercise_id+'">Rediger</button><button class="button secondary" data-del="'+e.exercise_id+'">Slett øvelse</button></div></div>'; }
      document.getElementById('exlist').innerHTML=html||'<div class="card small">Ingen øvelser matcher filteret.</div>';
    ...
    function matchesFilters(e){
      ...
      if(fFocus&&e.focus_area!==fFocus) return false;
      if(fCat&&e.category!==fCat) return false;
      const eInt=(e.rpe_text==='Lett'?'Lav':e.rpe_text==='Moderat'?'Middels':'Høy'); if(fInt&&eInt!==fInt) return false;
      if(fNoise&&(e.noise_level||'Medium')!==fNoise) return false;
      const sel=[...fSelectedEquip]; const req=(e.equipment||[]).filter(x=>x!=='nei');
      if(!sel.length&&req.length) return false; if(sel.length&&!req.every(x=>sel.includes(x))) return false;
      const text=(String(e.name||'')+' '+String(e.description||'')).toLowerCase(); if(fSearch&&!text.includes(fSearch)) return false;
      return true;
    }

    function renderItems(){ /* definert lenger opp */ }
