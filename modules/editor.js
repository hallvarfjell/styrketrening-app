...
// modules/editor.js
//
// Full Editor som oppfyller kravene:
// - BYGG ØKT: Øktnavn full bredde, Fokusområde (datalist) før Kategori, per-øvelse Varighet (mm:ss) + Pause (s),
//             drag & drop for rekkefølge, lagring som oppdaterer AppState.workouts (nye økter øverst).
// - LEGG TIL ØVELSE: Navn/Beskrivelse riktig bredde, Varighet (s) + Varighet pause (s), Fokus/Kategori (med datalist),
//                    Intensitet (Lav/Middels/Høy), Lydnivå (Lavt/Medium/Høyt – ingen fritekst), Utstyr (multiselect + legg til nytt).
//                    Validering: Navn, Beskrivelse, Intensitet, Kategori, Fokusområde, Lydnivå (ingen autom. defaulting).
// - FILTER ØVELSER: Fokus/Kategori/Intensitet/Lydnivå (Lydnivå rekkefølge Lavt–Medium–Høyt), Utstyr multiselect (riktig hint), Søk.
// - IMPORT/EKSPORT (exercises.csv): inkluderer default_pause_sec og intensitet (Lav/Middels/Høy) – bakoverkompatibel lesing fra rpe/rpe_text.
// - Ryddig og robust: ingen nested template literals, klar rekkefølge på hjelpefunksjoner, global eksport.
...
const Editor = {
  render(existing = null) {
    ...
    // ---------- Oppslagslister fra biblioteket ----------
    const eqSet = new Set();
    ...
    // Lydnivå i rekkefølge Lavt–Medium–Høyt + ev. andre bak
    function orderNoise(arr) {
      const pref = ['Lavt','Medium','Høyt'];
      const s = new Set(arr);
      const out = [];
      pref.forEach(p => { if (s.has(p)) out.push(p); });
      arr.forEach(x => { if (!out.includes(x)) out.push(x); });
      return out;
    }
    const allNoise = orderNoise(Array.from(noiseSet).length ? Array.from(noiseSet) : ['Lavt','Medium','Høyt']);

    // ---------- Hjelpere for HTML-generering ----------
        ...
        '<div>' +
          '<div class="card">' +
            ...
            '<div class="flex">' +
              ...
              '<select id="fCat"   class="input"><option value="">Kategori (alle)</option>' +
    ...
    document.getElementById('eximport').onclick = () => {
      ...
      reader.onload = () => {
        ...
        AppState.exercises = rows.map(r => {
          ...
          // Intensitet: les "intensitet" (Lav/Middels/Høy) eller fallback fra rpe/rpe_text
          const intensCsv = (r.intensitet || '').trim(); // Lav / Middels / Høy
          let rpeText = intensCsv
            ? (intensCsv === 'Lav' ? 'Lett' : intensCsv === 'Middels' ? 'Moderat' : 'Hardt')
            : (r.rpe_text || (Number(r.rpe) <= 3 ? 'Lett' : Number(r.rpe) <= 6 ? 'Moderat' : 'Hardt'));
        ...
        Editor.render(w); // re-render for å oppdatere lister/datalist
      };
    ...
    document.getElementById('addExercise').onclick = () => {
      ...
      if (!intens) missing.push('Intensitet');
    ...
    function matchesFilters(e){
      ...
      // Utstyr: hvis ingen valgt → vis kun kroppsvekt (dvs. req.length === 0)
      const selected = Array.from(fSelectedEquip);
      const req = (e.equipment || []).filter(x => x !== 'nei');
      if (!selected.length && req.length) return false;
      if (selected.length && !req.every(x => selected.includes(x))) return false;
    ...
    function renderExercises(){
      ...
      document.querySelectorAll('[data-del]').forEach(b => b.onclick = () => {
        ...
        if (idx>=0 && confirm('Slette øvelsen?')) {
          ...
          Store.save(Store.keys.exercises, AppState.exercises);
          // Fjern fra alle økter + pågående økt
    ...
    // ---------- BYGG ØKT: items-liste med drag & drop ----------
    ...
    document.getElementById('save').onclick = () => {
      ...
      // Beregninger for computed
      const equipSet = new Set(); let rpeSum=0, rpeCount=0;
      const noiseLevels = { Low:1, Medium:2, High:3 };
      let noiseMax = 1;
