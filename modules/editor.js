...
const Editor = {
  render(existing = null) {
    ...
    const html =
      '<div class="grid-2">' +
        '<div>' +
          // BYGG ØKT (ens bredde med de andre) + lagre-ikon
          '<div class="card">' +
            '<h3>Bygg økt</h3>' +
            ...
            '<div class="flex" style="margin-top:8px;">' +
              '<button class="icon-btn" id="save" aria-label="Lagre økt"><svg class="icon"><use href="#ph-floppy-disk-fill"></use></svg></button>' +
            '</div>' +
          '</div>' +

          // LEGG / REDIGER ØVELSE – 2 kolonner × 3 rader + utstyr-velger
          '<div class="card">' +
        ...
        // HØYRE: ØVELSER + FILTER (reintrodusert utstyrsfilter)
        '<div>' +
          '<div class="card">' +
            '<h3>Øvelser</h3>' +
            '<div class="flex">' +
    ...
    document.getElementById('exportEx').onclick=()=>{
      ...
      const csv=Util.toCSV(headers,rows,';'); Util.download('exercises.csv', csv, 'text/csv;charset=utf-8');
    };
    ...
    function renderExercises(){
      ...
      document.querySelectorAll('[data-add]').forEach(b=>b.onclick=()=>{
        ...
        const d=Number(ex?.default_duration_sec||60), p=Number(ex?.default_pause_sec||10);
    ...
    // Drag & drop (håndtak på høyre side)
    function addDragHandlers(card, idx){
      ...
      handle && (handle.onmousedown = ()=>{}); // mark handle as grip (visual only)
    }
      ...
      w.items.forEach((it, idx) => {
        ...
        html+='<div class="card item-card" data-idx="'+idx+'">'+
                ...
                '<div class="flex" style="margin-top:6px;">'+
                  '<div style="flex:1;"><div class="small" style="color:#666;margin-bottom:4px;">Varighet (mm:ss)</div><input class="input" value="'+Util.fmtMMSS(d)+'" data-dur="'+idx+'"/></div>'+
                  '<div style="flex:1;"><div class="small" style="color:#666;margin-bottom:4px;">Pause (s)</div><input class="input" value="'+p+'" data-pause="'+idx+'"/></div>'+
                '</div>'+
                '<div class="flex" style="margin-top:8px;">'+
                  '<button class="icon-btn trash" aria-label="Fjern" data-del="'+idx+'"><svg class="icon"><use href="#ph-trash-fill"></use></svg></button>'+
                '</div>'+
              '</div>';
      });
    ...
    document.getElementById('save').onclick=()=>{
      ...
      w.name=name; w.focus_area=focus; w.category=cat; if(!w.created_at) w.created_at=Date.now();
