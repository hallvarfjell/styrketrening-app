...
const Library = {
  render(){
    ...
    // Utstyr fra øvelser
    const eqSet = new Set(); (AppState.exercises||[]).forEach(e=>(e.equipment||[]).forEach(x=>{ if(x) eqSet.add(x); }));
    ...
    render(
      '<div class="card">' +
        ...
        <!-- Utstyr: dropdown multivalg (gjeninnført) -->
    ...
    function refreshList(){
      ...
      list = list.filter(w => {
        ...
        if (!sel.length) return req.length === 0;
        return req.every(eq => sel.includes(eq));
      });
      ...
      const html = list.map(w => {
        ...
        return (
          '<div class="card">' +
            '<div class="row">' +
              '<div class="title"><strong>'+w.name+'</strong> <span class="small">'+(w.category||'')+' • '+(w.focus_area||'')+'</span><div class="small">Utstyr: '+(reqEquip.length?reqEquip.join(', '):'ingen')+'</div></div>' +
              '<div class="actions">' +
                '<button class="icon-btn play" aria-label="Start" data-start="'+w.workout_id+'"><svg class="icon"><use href="#ph-play-fill"></use></svg></button>' +
                '<button class="icon-btn" aria-label="Rediger" data-edit="'+w.workout_id+'"><svg class="icon"><use href="#ph-pencil-fill"></use></svg></button>' +
                '<button class="icon-btn trash" aria-label="Slett" data-del="'+w.workout_id+'"><svg class="icon"><use href="#ph-trash-fill"></use></svg></button>' +
                '<button class="icon-btn fav '+(w.favorite?'active':'')+'" aria-label="Favoritt" data-fav="'+w.workout_id+'">' +
                  (w.favorite
                    ? '<svg class="icon"><use href="#ph-star-fill"></use></svg>'
                    : '<svg class="icon"><use href="#ph-star"></use></svg>') +
                '</button>' +
              '</div>' +
            '</div>' +
          '</div>'
